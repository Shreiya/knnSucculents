...
function gotResults(err, result) {
  ...
  if (result.confidencesByLabel) {
    ...
    select('#confidenceRock').html(`${confideces['Howard'] ? confideces['Howard'] * 100 : 0} %`);
    select('#confidencePaper').html(`${confideces['Sadie'] ? confideces['Sadie'] * 100 : 0} %`);
    select('#confidenceScissor').html(`${confideces['Evie'] ? confideces['Evie'] * 100 : 0} %`);
  }
